...
export default function Video() {
    ...
    return (
        <div className={styles.alignment} style={{ backgroundColor: '#000', padding: '20px', borderRadius: '10px' }}>
    <h1 className={styles.title} style={{ color: 'white' }}>New Video</h1>
    <input
    ...
    <>
        ...
        <div
            ...
            style={{ backgroundColor: '#333', color: 'white', border: '2px dashed white' }}
        >
            <p>Drag your video here...</p>
        </div>
        <p style={{ color: 'white' }}>{videoFile ? videoFile?.name : null}</p>
    </>
    {/*Select Resolutions and Worker*/}
    <div className={styles.selectContainer}>
        <div className={styles.selectWrapper}>
            <label className={styles.selectLabel} style={{ color: 'white' }}>Select Resolutions:</label>
            <Listbox>
                {({ open }) => (
                    <>
                        <div className={styles.multiSelectDisplay}>
                            <Listbox.Button className={styles.listBox} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>
                                {selectedResolutions.length > 0 ? selectedResolutions.map(resolution => (
                                    <span key={resolution} className={styles.selectedItem} style={{ color: 'white' }}>
                        ...
                        <Transition show={open}>
                            <Listbox.Options className={styles.optionsBox} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>
        ...
        <div className={styles.selectWrapper}>
            <label className={styles.selectLabel} style={{ color: 'white' }}>Select a worker:</label>
            <Listbox as="div" value={selectedWorker} onChange={setSelectedWorker}>
                {({ open }) => (
                    <>
                        <Listbox.Button className={styles.listBoxWorker} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>{selectedWorker}</Listbox.Button>
                        <Transition show={open}>
                            <Listbox.Options className={styles.optionsBox} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>
                                {workers.map((worker) => (
                                    <Listbox.Option className={styles.options} key={worker} value={worker}>
                                        {({ selected }) => (
                                            <div className={cx(styles.option, selected ? styles.selectedOption : null)} style={{ color: 'white' }}>
                                                <span>{worker}</span>
                                            </div>
                                        )}
                                    </Listbox.Option>
    ...
    <h4 style={{ color: 'white' }}>Stored in metadata</h4>
    <input
        className={styles.videoURL}
        type="text"
    ...
    />
    {/*Enable DRM via NFTs*/}
    <h4 style={{ color: 'white' }}>Enable NFT based DRM with NFT collection address (optional)</h4>
    {collections.map((collection, index) => (
        <div key={index} className={styles.collectionRow}>
            <div className={styles.collectionInputs}>
                <div className={styles.inputGroup}>
                    <label style={{ color: 'white' }}>Enter Collection#{index + 1} Address:</label>
                    <input
                ...
                <div className={styles.inputGroup}>
                    <label style={{ color: 'white' }}>Select Collection#{index + 1} Network:</label>
                    <Listbox as="div" value={collection.network} onChange={(network) => handleNetworkChange(index, network)}>
                        {({ open }) => (
                            <>
                                <Listbox.Button className={styles.listBoxNetwork} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>{collection.network}</Listbox.Button>
                                <Transition show={open}>
                                    <Listbox.Options className={styles.optionsBoxNetwork} style={{ backgroundColor: '#333', color: 'white', border: '1px solid white' }}>
            ...
            <div style={{ display: "flex", justifyContent: "end", alignItems: "center" }}>
                <button className={styles.buttonRemove} onClick={() => handleRemoveCollection(index)} style={{ backgroundColor: '#555', color: 'white', border: '1px solid white' }}>Remove</button>
            </div>
        </div>
    ))}
    <button className={styles.basicButton} onClick={handleAddCollection} style={{ backgroundColor: '#555', color: 'white', border: '1px solid white' }}>Add another NFT collection</button>
    ...
    {/*Save button, loading animation and Error messages*/}
    {
        isUploading ? (
            ...
            <button className={styles.basicButton} onClick={handleSaveVideo} style={{ backgroundColor: '#555', color: 'white', border: '1px solid white' }}>
                Save
            </button>
        )
    }
...
function ApiKeys({ setApiKeys }: ApiKeysProps): JSX.Element {
    ...
    <h1 className={styles.title} style={{ color: 'white' }}>Enter API Keys</h1>
    <div className={styles.inputWrapper}>
        <label style={{ color: 'white' }}>API Key:</label>
        <input
    ...
    <div className={styles.secretWrapper}>
        <label style={{ color: 'white' }}>API Secret:</label>
        <input
            className={styles.apiKeys}
            type="text"
    ...
    <button className={styles.basicButton} onClick={handleSaveKeys} style={{ backgroundColor: '#555', color: 'white', border: '1px solid white' }}>Save Keys</button>
    <p id="keysError" className={styles.errorText} style={{ color: 'red' }}>{keysErrorMessage}</p>
</div>
